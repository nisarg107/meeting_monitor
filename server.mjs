...
async function connectMongo() {
  ...
  try {
    ...
    const connectPromise = mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/zoom-clone');
...
app.prepare().then(async () => {
  // Connect to MongoDB (optional - won't block startup)
  connectMongo().catch(err => {
    console.log('⚠️ MongoDB connection failed, continuing without database:', err.message);
  });
